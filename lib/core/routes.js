...
function routes(app) {
   ...
   * @description routes created to work with social media integration, such as twitter, facebook and instagram.
   */
  app.get('/keystone/auth/youtube', function(req, res) {
    var domain = keystone.get('env') == 'production' ? 'http://costao.mktbt.com' : 'http://localhost:3000';
    var url = 'https://accounts.google.com/o/oauth2/auth?client_id=867553353887-ogkmdg74kjbg6mhqs6m21jn11ap5ga2n.apps.googleusercontent.com&redirect_uri=' + domain + '/keystone/medias/youtube&scope=https://www.googleapis.com/auth/youtube&response_type=code&access_type=offline'
    res.redirect(url);
  })
  app.get('/keystone/medias', require('../../admin/routes/views/medias'));
  app.get('/keystone/medias/twitter', require('../../admin/routes/views/mediatwitter'));
  app.get('/keystone/medias/instagram', require('../../admin/routes/views/mediainstagram'));
  app.get('/keystone/medias/facebook', require('../../admin/routes/views/mediafacebook'));
  app.get('/keystone/medias/youtube', require('../../admin/routes/views/mediayoutube'));
