...
/**
 * s3multiple FieldType Constructor
 * @extends Field
 * @api public
 */

function s3multiple(list, path, options) {
  ...
  if (options.initial) {
    throw new Error('Invalid Configuration\n\n' +
      's3multiple fields (' + list.key + '.' + path + ') do not currently support being used as initial fields.\n');
  }

  s3multiple.super_.call(this, list, path, options);

  // validate s3 config (has to happen after super_.call)
  if (!this.s3config) {
    throw new Error('Invalid Configuration\n\n' +
      's3multiple fields (' + list.key + '.' + path + ') require the "s3 config" option to be set.\n\n' +
...
util.inherits(s3multiple, super_);

/**
 * Exposes the custom or keystone s3 config settings
 */

Object.defineProperty(s3multiple.prototype, 's3config', {
  get: function() {
...
s3multiple.prototype.addToSchema = function() {
  ...
  var schemaMethods = {
    ...
    /**
     * Deletes the file from s3multiple and resets the field
     *
     * @api public
     */
...
s3multiple.prototype.format = function(item) {
...
s3multiple.prototype.hasFormatter = function() {
...
s3multiple.prototype.isModified = function(item) {
...
s3multiple.prototype.validateInput = function(data) {//eslint-disable-line no-unused-vars
...
s3multiple.prototype.updateItem = function(item, data) {//eslint-disable-line no-unused-vars
...
s3multiple.prototype.generateHeaders = function (item, file, callback){
  var field = this,
...
s3multiple.prototype.uploadFile = function(item, file, update, callback) {
 ...
 * - `field.paths.upload` in `req.files` (uploads the file to s3multiple)
 *
 * @api public
 */

s3multiple.prototype.getRequestHandler = function(item, req, paths, callback) {
...
s3multiple.prototype.handleRequest = function(item, req, paths, callback) {
...
exports = module.exports = s3multiple;
